...
export default function Card(props: Props) {
    ...
    return (
        <CardContainer
            className={`w-[${
                isScreen472 ? '80px' : '300px'
            }] bg-gradient-to-b from-[#2A3169] to-[#141839] rounded-[20px]`}
        >
            <img
            ...
            <div className="py-3 px-3">
                <div className="flex flex-row justify-between items-center">
                    <Text
                        isScreenSm={isScreen472}
                        className="text-[#C2C2C2] font-bold grow"
                    >
                        {name}
                    </Text>
                    <p
                        ...
                        {`${seller}`.toUpperCase() !== `${address}`.toUpperCase() ? (
                            <Web3Button
                                ...
                                action={async contract => {
                                    await contract.call('buy', [listingId], {
                                        value: price.raw,
                                    });
                                }}
                                className={`text-[${
                                    isScreen472 ? '18px' : '24px'
                                }] w-full font-black uppercase rounded-b-[20px] rounded-t-[0px] text-white bg-gradient-to-b from-[#ECB602] to-[#EC7202]`}
